...
│  ├─ state.js              内存状态与变更操作
│  ├─ auth.js               家长锁（密码哈希、恢复码、记住密码）
│  ├─ chart.js              SVG 柱状图
│  ├─ util.js               工具函数（转义、提示、下载等）
│  └─ views/                各页面视图：check / shop / ledger / me / manage / manage-goods / auth
├─ icons/                   PWA 图标（192 / 512 / maskable-512）
├─ tests/                   Node 内置测试器用例（store / state / chart / util / auth / views）
├─ tools/
...
- **首次打开**：设置 4–6 位数字密码，然后抄写屏幕上的 **8 位恢复码**（忘记密码时用它重设，旧码用一次即作废）。
- **日常打开**：输入密码进入；勾选「记住密码」后下次直接进入，直到在「我的 → 退出登录」退出。
- **忘记密码**：锁屏页点「忘记密码？」，输入恢复码重设新密码（会显示新的恢复码）。
- **修改与关闭**：「我的」页可修改密码或退出登录；未设置过密码时显示「设置密码（家长锁）」入口。
- **说明**：密码以 PBKDF2-SHA256 哈希保存在本机，不进入导出备份；这是防孩子误操作的**本机锁**，不是数据加密——积分数据本身未加密，能接触设备并有技术手段者仍可绕过。
- 如果不想使用家长锁，首次设置页可以点「暂不设置，先使用」。
...
- 家长锁的密码与恢复码单独保存在本机，**不进入导出备份**；清除浏览器数据会同时清掉密码和积分数据，重装/换机前请先导出备份。
...
12. 家长锁·首次设置：首次打开出现设置密码页，设置后显示恢复码；抄写恢复码并点「我已抄写」进入应用。
13. 家长锁·日常进入：不勾「记住密码」时重新打开需输密码，输错有提示；勾选后重新打开直接进入；「我的 → 退出登录」后再次要求输密码。
14. 家长锁·找回与修改：「忘记密码？」输入恢复码可重设（旧恢复码失效）；「我的 → 修改密码」需先输当前密码，成功后显示新恢复码。
