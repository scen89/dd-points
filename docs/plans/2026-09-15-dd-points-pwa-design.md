...
## 功能增强：明细趋势图奖惩分开展示（2026-09-15 追加，已实现）

- 趋势图由单柱（净得分）改为**上下对称双柱**：橙色奖励柱向上、红色惩罚柱向下，零轴居中；每天一组，组间距大于组内间距
- 口径：只统计**打卡流水**（`source === 'task'`）；兑换属于消费，**不计入**奖惩柱与占比（图例旁标注「仅统计打卡，不含兑换」）
- 图表下方汇总：`近N天：奖励 X 分（P%）· 惩罚 Y 分（Q%）`
- 每日分组标题显示当天拆分：`奖励 +6 · 惩罚 -3 · 任务净 +3`（与图表同口径；打卡页「今日/本周」仍是含兑换的全部流水净额）

## 功能增强：家长锁（2026-09-15 追加，已实现，取代早期「不需要 PIN」决策）

- 打开应用需 4–6 位数字密码；勾选「记住密码」后本机自动登录，直到手动「退出登录」
- 首次使用显示设置密码页（可「暂不设置，先使用」；未设置时「我的」提供「设置密码」入口）
- 忘记密码用 8 位恢复码重设新密码，旧码即作废；未点击「我已抄写」确认前重新打开会重新生成恢复码
- 「我的」页可修改密码（需验证当前密码）与退出登录
- 安全实现：PBKDF2-SHA256（16 字节盐、10 万次迭代）存哈希，明文不落盘；认证数据存独立 key `dengdeng_auth_v1`，**不进入导出备份**
- 定位：本机威慑锁（防孩子误操作），非数据加密；积分数据本身未加密，具备技术手段者可绕过
- 登录表单使用标准 password 字段与 autocomplete，Chrome 密码管理器可保存并自动填充
