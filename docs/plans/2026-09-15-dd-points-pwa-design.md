...
## 功能增强：兑换数量与兑换记录（2026-09-15 追加，已实现）

- 流水新增可选 `count`（兑换数量，1–999 整数，缺省 1）；`points` 仍存**总扣分**（单价 × 数量），余额与图表逻辑不变
- 兑换确认弹窗内提供 `[− 数量 ＋]` 步进器：合计与兑换后余额实时显示；上限 = 余额可兑数量（同时不超过 999）
- 商城页右上角 📋 打开「兑换记录」：按时间倒序，顶部「共兑换 N 件 · 累计消耗 X 分」；纯查看
- 商品积分限制为不低于 0.01 且最多两位小数，避免凑整误差
